...
def fn_to_cmd(fn: Union[Callable, str], arity: int) -> Tuple[str, int]:
    ...
    return make_cmd(
        lambda var_names: f"{fn_name}({', '.join(var_names)}, ctx=CTX)", arity
    )
...
command_dict = {
    ...
    "⟇": make_cmd("{1} or {0}", 2),
    "÷": (
        "for item in iterable(pop(CTX.stack), ctx=CTX): CTX.stack.append(item)",
        1,
    ),
    "•": fn_to_cmd(log, 2),
    "†": (
    ...
    "+": fn_to_cmd(add, 2),
    ",": ("vy_print(pop(CTX.stack), ctx=CTX)", 1),
    ...
    "A": make_cmd("int(all(iterable({}, ctx=CTX)))", 1),
    ...
    "Z": make_cmd(
        "Generator(vy_zip(iterable({}, ctx=CTX), iterable({}, ctx=CTX)))",
        2,
    ),
    "a": ("CTX.stack.append(int(any(iterable(pop(CTX.stack), ctx=CTX))))", 1),
    "b": fn_to_cmd(vy_bin, 1),
    "c": (
        "needle, haystack = pop(CTX.stack, 2);"
        "haystack = iterable(haystack, str, ctx=CTX)\n"
        "if type(haystack) is str: needle = vy_str(needle)\n"
        "CTX.stack.append(int(needle in iterable(haystack, str, ctx=CTX)))",
        2,
    ),
    "d": make_cmd("multiply({}, 2)", 1),
    "e": fn_to_cmd(exponate, 2),
    "f": make_cmd("flatten(iterable({}, ctx=CTX))", 1),
    "g": make_cmd("vy_min(iterable({}, ctx=CTX))", 1),
    "h": make_cmd("iterable({}, ctx=CTX)[0]", 1),
    "i": fn_to_cmd(index, 2),
    ...
    "t": make_cmd("iterable({}, ctx=CTX)[-1]", 1),
    "u": make_cmd("-1", 0),
    "w": make_cmd("[{}]", 1),
    ...
    "ė": (
        "CTX.stack.append(Generator(enumerate(iterable(pop(CTX.stack), ctx=CTX))))",
        1,
    ),
    "ḟ": fn_to_cmd(find, 2),
    "ġ": (
    ...
    "ḣ": (
        "top = iterable(pop(CTX.stack), ctx=CTX); CTX.stack.append(top[0]); CTX.stack.append(top[1:])",
        1,
    ),
    ...
    "ṁ": (
        "top = iterable(pop(CTX.stack), ctx=CTX); CTX.stack.append(divide(summate(top), len(top)))",
        1,
    ),
    "ṅ": fn_to_cmd(first_n, 1),
    "ȯ": fn_to_cmd(first_n, 2),
    "ṗ": ("CTX.stack.append(powerset(iterable(pop(CTX.stack), ctx=CTX)))", 1),
    ...
    "ṫ": (
        "vector = iterable(pop(CTX.stack), ctx=CTX); CTX.stack.append(vector[:-1]); CTX.stack.append(vector[-1])",
        1,
    ),
    "ẇ": fn_to_cmd(wrap, 2),
    "ẋ": (
    ...
    "ẏ": (
        "obj = iterable(pop(CTX.stack), ctx=CTX); CTX.stack.append(Generator(range(0, len(obj))))",
        1,
    ),
    "ż": (
        "obj = iterable(pop(CTX.stack), ctx=CTX); CTX.stack.append(Generator(range(1, len(obj) + 1)))",
        1,
    ),
    ...
    "¦": (
        "CTX.stack.append(cumulative_sum(iterable(pop(CTX.stack), ctx=CTX)))",
        1,
    ),
    "≈": (
        "CTX.stack.append(int(len(set(iterable(pop(CTX.stack), ctx=CTX))) == 1))",
        1,
    ),
    "Ȧ": (
        "value, lst_index, vector = pop(CTX.stack, 3); CTX.stack.append(assigned(iterable(vector, ctx=CTX), lst_index, value))",
        3,
    ),
...
if vy_type(top) is Number:
    ...
    limit = -1; vector = top
fn = pop(CTX.stack)
CTX.stack.append(Generator(fn, limit=limit, initial=iterable(vector, ctx=CTX)))
""",
        2,
    ),
    "Ġ": make_cmd("group_consecutive(iterable({}, ctx=CTX))", 1),
    "Ḣ": ("CTX.stack.append(iterable(pop(CTX.stack), ctx=CTX)[1:])", 1),
    ...
    "Ṗ": (
        "CTX.stack.append(Generator(permutations(iterable(pop(CTX.stack), ctx=CTX))))",
        1,
    ),
    "Ṙ": fn_to_cmd(reverse, 1),
    "Ṡ": ("CTX.stack = [summate(CTX.stack)]", 0),
    "Ṫ": ("CTX.stack.append(iterable(pop(CTX.stack), str, ctx=CTX)[:-1])", 1),
    ...
    "¯": fn_to_cmd(deltas, 1),
    "±": fn_to_cmd(sign_of, 1),
    "₴": ("vy_print(pop(CTX.stack), end='', ctx=CTX)", 1),
    "…": (
        "top = pop(CTX.stack); CTX.stack.append(top); vy_print(top, ctx=CTX)",
        0,
    ),
    "□": (
    ...
    "℅": make_cmd("random.choice(iterable({}, ctx=CTX))", 1),
    ...
    "Π": ("CTX.stack.append(product(iterable(pop(CTX.stack), ctx=CTX)))", 1),
    "„": (
    ...
    "∆P": make_cmd("polynomial(iterable({}, ctx=CTX))", 1),
    ...
    "∆ṙ": (
        "CTX.stack.append(unsympy(sympy.prod(map(sympy.poly('x').__sub__, iterable(pop(CTX.stack), ctx=CTX))).all_coeffs()[::-1]))",
        1,
    ),
    ...
    "øm": make_cmd("palindromise(iterable({}), ctx=CTX)", 1),
    "øe": make_cmd(
        "run_length_encode(iterable({}, str, ctx=CTX))",
        1,
    ),
    ...
    "ÞU": make_cmd("nub_sieve(iterable({}, ctx=CTX))", 1),
    "ÞT": fn_to_cmd(transpose, 1),
    "ÞD": (
        "CTX.stack.append(Generator(diagonals(iterable(pop(CTX.stack), list, ctx=CTX))))",
        1,
    ),
    "ÞS": (
        "CTX.stack.append(Generator(sublists(iterable(pop(CTX.stack), list, ctx=CTX))))",
        1,
    ),
    "ÞṪ": (
        "rhs, lhs = pop(CTX.stack, 2); print(lhs, rhs) ;CTX.stack.append(Generator(itertools.zip_longest(*iterable(lhs, ctx=CTX), fillvalue=rhs)))",
        2,
    ),
    "Þ℅": (
    ...
    "¨M": (
        "function, indices, original = pop(CTX.stack, 3); CTX.stack.append(map_at(function, iterable(original, ctx=CTX), iterable(indices, ctx=CTX)))",
        3,
    ),
    "¨,": ("vy_print(pop(CTX.stack), end=' ', ctx=CTX)", 1),
    "¨…": (
        "top = pop(CTX.stack); CTX.stack.append(top); vy_print(top, end=' ', ctx=CTX)",
        1,
    ),
